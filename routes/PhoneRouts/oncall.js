...
router.get("/search", async (req, res) => {
  try {
    ...
    if (req.query.complaintType) {
      baseQuery["complaint.complaintType"] = new RegExp(
        req.query.complaintType,
        "i"
      );
    }
...
// OnCall Assignment List API for Orphaned/Reassignment OnCalls
router.get("/assign-list", async (req, res) => {
  try {
    ...
    // Build aggregation pipeline to find orphaned OnCalls
    const pipeline = [
      {
        $match: {
          onCallproposalstatus: "Open",
        },
      },
      // Stage 1: Lookup to get user details for createdBy field (employeeid lookup)
      {
        $lookup: {
          from: "users",
          localField: "createdBy", // OnCall.createdBy (employeeid)
          foreignField: "employeeid", // User.employeeid
          as: "createdByUser",
        },
      },
      {
      ...
      // Stage 2: Add computed fields to identify orphaned OnCalls
      {
        $addFields: {
          isOrphaned: {
            $or: [
              // CreatedBy user is deactivated
              { $eq: ["$createdByUser.status", "Deactive"] },

              // CreatedBy user login has expired
              {
                $and: [
                  { $ne: ["$createdByUser.loginexpirydate", null] },
                  { $lt: ["$createdByUser.loginexpirydate", "$$NOW"] },
                ],
              },
            ],
          },
          // Set assignment status based on createdBy user status
          computedAssignmentStatus: {
            $cond: {
              if: { $eq: ["$createdByUser.status", "Deactive"] },
              then: "orphaned_deactivated",
              else: {
                $cond: {
                  if: {
                    $and: [
      ...
      // Stage 3: Match only orphaned OnCalls (main filter)
      {
        $match: {
          isOrphaned: true,
        },
      ...
      pipeline.push({
        $match: {
          $or: [
            ...
            { "customer.customername": searchRegex },
            { "customer.customercode": searchRegex },
            { "productGroups.productPartNo": searchRegex },
            { "complaint.serialnumber": searchRegex },
            { createdBy: searchRegex },
          ],
        },
    ...
    if (statusFilter && statusFilter !== "all") {
      pipeline.push({
        $match: {
          computedAssignmentStatus: statusFilter,
        },
      });
    }
    ...
    const totalResult = await OnCall.aggregate(countPipeline);
    ...
    // Execute main query
    const onCalls = await OnCall.aggregate(pipeline);
    console.log("Orphaned OnCalls:", onCalls);

    // Transform the data for frontend
    const transformedData = onCalls.map((onCall) => ({
      _id: onCall._id,
      onCallNumber: onCall.onCallNumber,
      customer: onCall.customer,
      productGroups: onCall.productGroups,
      complaint: onCall.complaint,
      assignmentStatus: onCall.computedAssignmentStatus,
      createdBy: onCall.createdBy,
      createdByUser: onCall.createdByUser
        ? {
            _id: onCall.createdByUser._id,
            name: `${onCall.createdByUser.firstname} ${
              onCall.createdByUser.lastname || ""
            }`.trim(),
            email: onCall.createdByUser.email,
            employeeid: onCall.createdByUser.employeeid,
            status: onCall.createdByUser.status,
            loginexpirydate: onCall.createdByUser.loginexpirydate,
          }
        : null,
      priority: onCall.priority || "normal",
      createdAt: onCall.createdAt,
      isOrphaned: onCall.isOrphaned,
      // Why it's orphaned
      orphanReason: !onCall.createdByUser
        ? "User Not Found"
        : onCall.createdByUser?.status === "Deactive"
        ? "User Deactivated"
        : onCall.createdByUser?.loginexpirydate &&
          onCall.createdByUser.loginexpirydate < new Date()
        ? "Login Expired"
        : "Unknown",
    }));
    ...
    const response = {
      ...
      summary: {
        totalOrphanedOnCalls: total,
        orphanedReasons: {
          userDeactivated: transformedData.filter(
            (oc) => oc.orphanReason === "User Deactivated"
          ).length,
          loginExpired: transformedData.filter(
            (oc) => oc.orphanReason === "Login Expired"
          ).length,
          userNotFound: transformedData.filter(
            (oc) => oc.orphanReason === "User Not Found"
          ).length,
        },
      },
    };
    ...
    console.error("OnCall assign-list error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching orphaned OnCalls",
      error: error.message,
    });
  }
});
...
router.post("/:onCallId/assign", async (req, res) => {
  console.log("Assigning OnCall...");
  try {
    const { assignedTo, notes = "" } = req.body;
    const { onCallId } = req.params;

    // Verify the OnCall exists
    const onCallExists = await OnCall.findById(onCallId);
    if (!onCallExists) {
      return res.status(404).json({
        success: false,
        message: "OnCall not found",
        errorCode: "ONCALL_NOT_FOUND",
      });
    }
    ...
    console.log("assingedTo", assignedTo);

    // Update the OnCall document - Override createdBy with assigning user's employeeId
    const updatedOnCall = await OnCall.findByIdAndUpdate(
      onCallId,
      {
        $set: {
          createdBy: assignedTo, // Override createdBy field
    ...
    if (!updatedOnCall) {
      return res.status(404).json({
        success: false,
        message: "Failed to update OnCall",
    ...
    res.json({
      success: true,
      message: "OnCall assigned successfully",
      data: {
        _id: updatedOnCall._id,
        onCallNumber: updatedOnCall.onCallNumber,
        createdBy: updatedOnCall.createdBy, // Now contains assigning user's employeeId
      },
    });
  } catch (error) {
    console.error("OnCall assignment error:", error);
    res.status(500).json({
      success: false,
      message: "Error assigning OnCall",
      errorCode: "SERVER_ERROR",
      error: error.message,
    });
  }
...
router.put("/:id/revision", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to process revision", error: err.message });
  }
});
...
router.put("/:id/approve-rsh", async (req, res) => {
  try {
    ...
    if (!oncall)
      return res
        .status(404)
        .json({ success: false, message: "OnCall not found" });
      ...
      req.params.id,
      {
        ...
        $push: {
          statusHistory: {
            ...
            note:
              proposalRemark ||
              `Proposal status changed to ${onCallproposalstatus}`,
          },
        },
      },
