    ...
    if (!proposal) {
      ...
      if (lastError.code === 11000) {
        return res.status(409).json({
          message:
            "Failed to generate unique proposal number after multiple attempts",
...
router.get("/assign-list", async (req, res) => {
  try {
    ...
    // Build aggregation pipeline to find orphaned Proposals
    const pipeline = [
      {
        $match: {
          Cmcncmcsostatus: "Open", // Only Open CMC/NCMC proposals
        },
      },
      // Stage 1: Lookup to get user details for createdBy field (employeeid lookup)
      {
        $lookup: {
          from: "users",
          localField: "createdBy", // Proposal.createdBy (employeeid)
      ...
      // Stage 2: Add computed fields to identify orphaned Proposals
      {
        $addFields: {
          isOrphaned: {
            $or: [
              ...
              // CreatedBy user login has expired
              {
                $and: [
                  ...
                  { $ne: ["$createdByUser.loginexpirydate", undefined] },
                  { $lt: ["$createdByUser.loginexpirydate", currentDate] },
                ],
              },
            ],
          },
      ...
      // Stage 3: Match only orphaned Proposals (main filter)
      {
        $match: {
          isOrphaned: true,
        },
    ...
    const transformedData = proposals.map((proposal) => ({
      ...
      assignmentStatus: proposal.computedAssignmentStatus,
      createdBy: proposal.createdBy,
      createdByUser: proposal.createdByUser
        ? {
            _id: proposal.createdByUser._id,
            name: `${proposal.createdByUser.firstname} ${
              proposal.createdByUser.lastname || ""
            }`.trim(),
            email: proposal.createdByUser.email,
            employeeid: proposal.createdByUser.employeeid,
            status: proposal.createdByUser.status,
            loginexpirydate: proposal.createdByUser.loginexpirydate,
          }
        : null,
      finalAmount: proposal.finalAmount,
      discountPercentage: proposal.discountPercentage,
      Cmcncmcsostatus: proposal.Cmcncmcsostatus,
      createdAt: proposal.createdAt,
      isOrphaned: proposal.isOrphaned,
      // Why it's orphaned
      orphanReason: !proposal.createdByUser
        ? "User Not Found"
        : proposal.createdByUser?.status === "Deactive"
        ? "User Deactivated"
        : proposal.createdByUser?.loginexpirydate &&
          proposal.createdByUser.loginexpirydate < new Date()
        ? "Login Expired"
        : "Unknown",
    }));
    ...
    const response = {
      ...
      summary: {
        totalOrphanedProposals: total,
        orphanedReasons: {
          userDeactivated: transformedData.filter(
            (p) => p.orphanReason === "User Deactivated"
          ).length,
          loginExpired: transformedData.filter(
            (p) => p.orphanReason === "Login Expired"
          ).length,
          userNotFound: transformedData.filter(
            (p) => p.orphanReason === "User Not Found"
          ).length,
        },
      },
    };
    ...
    console.error("Proposal assign-list error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching orphaned proposals",
      error: error.message,
    });
  }
});
      ...
      req.params.id,
      {
        ...
        $push: {
          statusHistory: {
            ...
            note:
              proposalRemark || `Proposal status changed to ${Cmcncmcsostatus}`,
          },
        },
      },
      { new: true, runValidators: true }
...
router.put("/:id/update-conumber", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "CoNumber updated, status set to completed, Cmcncmcsostatus closed won",
      data: proposal,
    });
  } catch (error) {
...
router.put("/:id/revision", async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `
                <p>Dear CIC,</p>
                <p>A new revision (<strong>#${newRevisionNumber}</strong>) has been made for the proposal with ID <strong>${
        proposal._id
      }</strong>.</p>
    ...
    res.status(200).json({
      message: `Proposal revised successfully${
        emailSent ? " and email sent to CIC" : " but email failed to send"
      }.`,
      updatedProposal,
      emailSent,
    });
  ...
  try {
    ...
    for (const itemId of itemIds) {
      ...
      if (item) {
        ...
        if (
          (requiresNSHApproval && item.NSHApproval?.approved) ||
          !requiresNSHApproval
        ) {
    ...
    if (currentRevision) {
      ...
      if (!requiresNSHApproval) {
        const allRSHApproved = proposal.items.every(
        ...
        const allNSHApproved = proposal.items.every(
  ...
  try {
    ...
    for (const itemId of itemIds) {
      ...
      if (item) {
        ...
        const requiresNSHApproval = proposal.discountPercentage > 10;
        if (
          (requiresNSHApproval && item.RSHApproval?.approved) ||
          !requiresNSHApproval
        ) {
          item.equipment.status = "Approved";
        }
...
router.put("/:id/bulk-reject-revision", async (req, res) => {
  try {
    ...
    const rejectedRSHItems = proposal.items.filter(
      (item) => item.RSHApproval?.rejected
    ).length;
    const rejectedNSHItems = proposal.items.filter(
      (item) => item.NSHApproval?.rejected
    ).length;
